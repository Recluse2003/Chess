...
namespace Chess.Application.Games.Commands.JoinGame
{
    ...
    public class JoinGameCommandHandler : IRequestHandler<JoinGameCommand, Result<Guid>>
    {
        ...
        /// attempting to join exists, is accepting an opponent, and that the player also exists.
        /// </summary>
        ...
        /// <returns>
        /// A successful join game request will provide a <see cref="Result"/> stating so, and contains the gameId. 
        /// A failure will provide a <see cref="Result"/> containing an <see cref="Error"/> stating the reason why.
        /// </returns>
        public async Task<Result<Guid>> Handle(JoinGameCommand command, CancellationToken cancellationToken)
        {
            try
            {
                Guid? chessGameId = await _gameRepository.GetGameIdByCodeAsync(command.Code);
                if (chessGameId == null)
                    return Error.NotFound("Games.InvalidCode", "The provided game code was not linked to any active game.");
